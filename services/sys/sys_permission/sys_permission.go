...
func Create(Permission *sys.SysPermission) (*sys.SysPermission, error) {
	ret, err := repo.Create(Permission)
	return ret, err
}

func UpdateById(id uint, Permission *sys.SysPermission) (*sys.SysPermission, error) {
...
func recursion(pid uint, list []sys.SysPermission, isRoot bool) []*TreeList {
	treeList := []*TreeList{}
	for _, v := range list {
		var isPass bool
		if isRoot {
