...
// Create 创建用户
//
//	func (r *UserRepository) Create(t *sys.SysUser, RoleId uint) (*sys.SysUser, error) {
...
//		return t, err
//	}
func (r *UserRepository) Create(t *sys.SysUser) (*sys.SysUser, error) {
	_db, err := db.GetMysql()
	if err != nil {
		return nil, err
	}
...
// UpdateById 修改用户
func (r *UserRepository) UpdateById(id uint, t *sys.SysUser) (*sys.SysUser, error) {
	var ret = new(sys.SysUser)
	_db, err := db.GetMysql()
	if err != nil {
		return nil, err
	}
...
// Get 获取用户列表
func (r *UserRepository) Get(page int, pageSize int, username string) ([]sys.SysUser, error) {
	var users []sys.SysUser
	var err error
	if pageSize < 1 {
		pageSize = 10
	}
...
// GetById 获取用户
func (r *UserRepository) GetById(id uint) (*sys.SysUser, error) {
	var ret = &sys.SysUser{}
	_db, err := db.GetMysql()
	if err != nil {
		return ret, err
...
// GetByName
func (r *UserRepository) GetByUserName(username string) (*sys.SysUser, error) {
	var ret = &sys.SysUser{}
	_db, err := db.GetMysql()
	if err != nil {
		return nil, err
